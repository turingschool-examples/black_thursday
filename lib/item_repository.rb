...
class ItemRepository
  ...
  def create(attributes)
    merchant_id = items.max { |item| item.id }
  ...
  def update(id, attributes)
    ...
    if item_by_id != nil
      item_by_id.change_name(attributes[:name])
      item_by_id.change_unit_price(attributes[:unit_price])
      item_by_id.change_description(attributes[:description])
      item_by_id.change_updated_at(attributes[:updated_at])
      require "pry"; binding.pry
    end
  end
