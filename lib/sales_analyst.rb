...
class SalesAnalyst
  ...
  def average_item_price_for_merchant(merchant_id)
    #cannot change
    float_answer = merchants_items_prices(merchant_id).reduce(:+) / merchants_items_prices(merchant_id).count
    (float_answer / 100).round(2)
  end

  def all_merchants_averages
    all_merchants_ids = se.merchants.all.map(&:id)
    all_merchants_ids.map { |merchant_id| average_item_price_for_merchant(merchant_id)}
  end
