...
class SalesAnalyst
  ...
  def top_buyers(selected = 20)
    ...
    customer_ids = cust_ids_sorted_by_total.map {|cust_id, total| cust_id}
  ...
  def highest_volume_items(customer_id)
    ...
    sorted_items = quantity_per_item.sort_by {|item_id, quantity| quantity}
  ...
  def best_invoice_by_quantity
    ...
    best_invoice_id = invoice_ids_and_quants.max_by {|inv_id, total| total}[0]
    @parent.invoices.find_by_id(best_invoice_id)
  end
end
