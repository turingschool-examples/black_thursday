...
class TransactionRepository < Repository
  ...
  def update(id, attributes)
    ...
    if update_instance.nil?
      nil
    elsif !attributes[:credit_card_number].nil?
      update_instance.credit_card_number = attributes[:credit_card_number]
      update_instance.updated_at = Time.now
    elsif !attributes[:credit_card_expiration_date].nil?
      update_instance.credit_card_expiration_date = attributes[:credit_card_expiration_date]
      update_instance.updated_at = Time.now
    elsif !attributes[:result].nil?
      update_instance.result = attributes[:result].to_sym
      update_instance.updated_at = Time.now
    end
  end
