...
class SalesEngine
  ...
  def self.from_csv(csv_data_paths)
    merch_repo        =
      MerchantRepository.new(csv_data_paths[:merchants])if csv_data_paths[:merchants]
    item_repo         =
      ItemRepository.new(csv_data_paths[:items]) if csv_data_paths[:items]
    invoice_repo      =
      InvoiceRepository.new(csv_data_paths[:invoices]) if csv_data_paths[:invoices]
    invoice_item_repo =
      InvoiceItemRepository.new(csv_data_paths[:invoice_items]) if csv_data_paths[:invoice_items]
    transaction_repo  =
      TransactionRepository.new(csv_data_paths[:transactions]) if csv_data_paths[:transactions]
    customer_repo     =
      CustomerRepository.new(csv_data_paths[:customers]) if csv_data_paths[:customers]
