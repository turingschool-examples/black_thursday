...
class InvoiceRepoTest < Minitest::Test
  ...
  @invoive_repo = InvoiceRepo(invoice_array)
end

  def test_it_exists
    assert_instance_of InvoiceRepo, @invoive_repo
  end  
  ...
  def test_it_returns_all_invoices_by_cutomer_id
  ...
  def test_it_returns_all_invoices_by_merchant_id
  ...
  def test_it_creates_a_new_invoice
    refute_instance_of Invoice, 
    @invoice_repo.invoices[14]
    @invoice_repo.create({
                id:           6,
                customer_id:  7,
                merchant_id:  8,
  ...
  def test_it_updates_invoice_attributes
    refute_equal "returned",
    @invoice_repo.invoices[13].status 
