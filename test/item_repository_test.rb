...
class ItemRepositoryTest < Minitest::Test
  ...
  def test_it_can_find_all_descriptions_that_match_supplied_fragment
    assert_instance_of Item, ir.find_all_with_description("original UI").first
    assert_equal 2, ir.find_all_with_description("colour").count
    assert_equal 263395617, ir.find_all_with_description("colour").first.id
  end
  ...
  def test_it_returns_instance_of_item_when_supplied_with_price
    assert_instance_of Item, ir.find_all_by_price(12.00).first
    assert_equal "510+ RealPush Icon Set", ir.find_all_by_price(12.00).first.name

    assert_equal "Disney scrabble frames", ir.find_all_by_price(13.50).first.name
  end
  ...
  def test_it_finds_all_in_price_range
    assert_equal 2, ir.find_all_by_price_in_range((10.00..13.00)).count
  end
