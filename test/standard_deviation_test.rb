...
class StandardDeviationTest < Minitest::Test
  ...
  def test_item_number_plus_one_deviation
    assert_equal 0.4, @sa.item_number_plus_one_deviation
  end

  def test_two_standard_deviations_away_in_price
    assert_equal 112.48, @sa.two_standard_deviations_away_in_price.to_f
  end

  def test_one_standard_deviation_above_invoice_average
    assert_equal 0.30, @sa.one_standard_deviation_above_invoice_average.round(2)
  end

  def test_one_standard_deviation_below_invoice_average
    assert_equal -0.26, @sa.one_standard_deviation_below_invoice_average
  end
