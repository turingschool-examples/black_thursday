...
class MerchantRepositoryTest < Minitest::Test
  ...
  def test_method_find_all_customers_by_merchant_id_returns_customers
    mock_se = Minitest::Mock.new
    mr = MerchantRepository.new([], mock_se)
    mock_se.expect(:find_all_customers_by_merchant_id, nil, [1000])
    mr.find_all_customers_by_merchant_id(1000)
    assert mock_se.verify
  end
end
