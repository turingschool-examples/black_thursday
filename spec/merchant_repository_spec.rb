...
describe MerchantRepository do
  ...
  it "can find merchant by name(case insensitive) and return nil if not found " do
    expect(@merchant_repository.find_all_by_name("Cand")).to be_a Array
    expect(@merchant_repository.find_all_by_name("Cand").length).to eq(5)
    expect(@merchant_repository.find_all_by_name("Cand").first.id).to eq(12334112)
    expect(@merchant_repository.find_all_by_name("jake")).to eq([])
  end

  it "can find all that will have the same name fragment" do
    expect(@merchant_repository.find_by_name("MotankiDarena").id).to eq(12334146)
    expect(@merchant_repository.find_by_name("dojdeo")).to eq(nil)
  end
