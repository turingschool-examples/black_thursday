...
RSpec.configure do |config|
  ...
  config.before(:suite) do
    BlackThursdaySpecHelpers.engine = SalesEngine.from_csv({
                                                             ...
                                                             merchants: File.join(spec_harness_root, 'data',
                                                                                  'merchants.csv'),
                                                             customers: File.join(spec_harness_root, 'data',
                                                                                  'customers.csv'),
                                                             invoices: File.join(spec_harness_root, 'data',
                                                                                 'invoices.csv'),
                                                             invoice_items: File.join(spec_harness_root, 'data',
                                                                                      'invoice_items.csv'),
                                                             transactions: File.join(spec_harness_root, 'data',
                                                                                     'transactions.csv')
                                                           })
  end
