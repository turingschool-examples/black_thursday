...
RSpec.describe InvoiceItemRepository do
  ...
  describe '#all_ids' do
    it 'will return all ids' do
      expect(invoice_item_repository.all_ids).to eq([6, 4, 8])
    end
  end

  describe '#update' do
    it 'will update invoice item' do
      original_time = invoice_item_repository.find_by_id(8).update_time

      invoice_item_repository.update(8, {:id => 8, :quantity => 7})
      expect(invoice_item_repository.find_by_id(8).quantity).to eq(7)

      invoice_item_repository.update(8, {:id => 8, :unit_price => BigDecimal(68.22, 4)})
      expect(invoice_item_repository.find_by_id(8).unit_price).to eq(0.6822e2)

      expect(invoice_item_repository.find_by_id(8).updated_at).to be > original_time
    end
  end
