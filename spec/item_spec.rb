  ...
  RSpec.describe Item do
    ...
    context 'Item' do
      it 'can read Item attributes' do
        ...
        expect(@item.unit_price).to eq(49.95)
        expect(@item.merchant_id).to eq(12334303)
        expect(@item.created_at).to eq(Time.parse("2016-01-11 11:23:02 UTC"))
        expect(@item.updated_at).to eq(Time.parse("2012-10-27 09:12:45 UTC"))
      end

      it '#unit_price_to_dollars can return price of items in dollars as float' do
        expect(@item.unit_price_to_dollars).to eq(49.95)
      end
